...
static input int    Magic_Number = 2125548912; // Magic number
static input double Entry_Amount =     1.00; // Entry lots
       input int    Stop_Loss    =       22; // Stop Loss   (pips)
       input int    Take_Profit  =       66; // Take Profit (pips)

static input string ___0______   = "------"; // --- Moving Average ---
       input int    Ind0Param0   =       53; // Period
       input int    Ind0Param1   =        0; // Shift

static input string ___1______   = "------"; // --- Alligator ---
       input int    Ind1Param0   =       21; // Jaws period
       input int    Ind1Param1   =       19; // Jaws shift
       input int    Ind1Param2   =        7; // Teeth period
       input int    Ind1Param3   =       14; // Teeth shift
       input int    Ind1Param4   =       14; // Lips period
       input int    Ind1Param5   =        5; // Lips shift

static input string ___2______   = "------"; // --- Candle Color ---
       input int    Ind2Param0   =       11; // Min body height [pips]
       input int    Ind2Param1   =        2; // Consecutive candles
...
static input string News_Currencies   = "USD,CHF,EUR"; // News currencies
...
void InitIndicators(void)
  {
   // Moving Average (Simple, Close, 53, 0)
   indHandlers[0][0][0] = iMA(NULL, 0, Ind0Param0, Ind0Param1, MODE_SMA, PRICE_CLOSE);
   if(Show_inds) ChartIndicatorAdd(0, 0, indHandlers[0][0][0]);
   // Alligator (Smoothed, Median, 21, 19, 7, 14, 14, 5)
...
int GetEntrySignal(void)
  {
   ...
   double ind1buffer2[]; CopyBuffer(indHandlers[0][1][0], 2, 1, 2, ind1buffer2);
...
//+------------------------------------------------------------------+
/*STRATEGY MARKET Premium Data; USDCHF; M5 */
/*STRATEGY CODE {"properties":{"entryLots":1,"tradeDirectionMode":0,"oppositeEntrySignal":1,"stopLoss":22,"takeProfit":66,"useStopLoss":true,"useTakeProfit":true,"isTrailingStop":false},"openFilters":[{"name":"Moving Average","listIndexes":[5,0,3,0,0],"numValues":[53,0,0,0,0,0]},{"name":"Alligator","listIndexes":[7,3,4,0,0],"numValues":[21,19,7,14,14,5]}],"closeFilters":[{"name":"Candle Color","listIndexes":[0,0,0,0,0],"numValues":[11,2,0,0,0,0]}]} */
