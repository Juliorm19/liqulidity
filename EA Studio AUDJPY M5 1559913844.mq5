...
static input int    Magic_Number = 1559913844; // Magic number
static input double Entry_Amount =     1.00; // Entry lots
       input int    Stop_Loss    =       34; // Stop Loss   (pips)
       input int    Take_Profit  =       49; // Take Profit (pips)

static input string ___0______   = "------"; // --- Stochastic Signal ---
       input int    Ind0Param0   =       16; // %K Period
       input int    Ind0Param1   =       12; // %D Period
       input int    Ind0Param2   =        3; // Slowing

static input string ___1______   = "------"; // --- ADX ---
       input int    Ind1Param0   =       27; // Period
       input double Ind1Param1   =      0.0; // Level

static input string ___2______   = "------"; // --- Bulls Power ---
       input int    Ind2Param0   =       30; // Period
       input double Ind2Param1   =  -0.0008; // Level

static input string ___3______   = "------"; // --- Bears Power ---
       input int    Ind3Param0   =        6; // Period
       input double Ind3Param1   =   0.0000; // Level

static input string ___4______   = "------"; // --- Envelopes ---
       input int    Ind4Param0   =       52; // Period
       input double Ind4Param1   =     0.35; // Deviation %

static input string ___5______   = "------"; // --- Accelerator Oscillator ---
       input double Ind5Param0   =   0.6779; // Level
...
static input string News_Currencies   = "AUD,JPY,USD,EUR"; // News currencies
...
void InitIndicators(void)
  {
   // Stochastic Signal (16, 12, 3)
   indHandlers[0][0][0] = iStochastic(NULL, 0, Ind0Param0, Ind0Param1, Ind0Param2, MODE_SMA, STO_LOWHIGH);
   if(Show_inds) ChartIndicatorAdd(0, 1, indHandlers[0][0][0]);
   // ADX (27)
   indHandlers[0][1][0] = iADX(NULL, 0, Ind1Param0);
   if(Show_inds) ChartIndicatorAdd(0, 2, indHandlers[0][1][0]);
   // Bulls Power (30), Level: -0.0008
   indHandlers[0][2][0] = iBullsPower(NULL, 0, Ind2Param0);
   if(Show_inds) ChartIndicatorAdd(0, 3, indHandlers[0][2][0]);
   // Bears Power (6)
   indHandlers[0][3][0] = iBearsPower(NULL, 0, Ind3Param0);
   if(Show_inds) ChartIndicatorAdd(0, 4, indHandlers[0][3][0]);
   // Envelopes (Close, Simple, 52, 0.35)
   indHandlers[0][4][0] = iEnvelopes(NULL, 0, Ind4Param0, 0, MODE_SMA, PRICE_CLOSE, Ind4Param1);
   if(Show_inds) ChartIndicatorAdd(0, 0, indHandlers[0][4][0]);
   // Accelerator Oscillator, Level: 0.6779
   indHandlers[0][5][0] = iAC(NULL, 0);
   if(Show_inds) ChartIndicatorAdd(0, 5, indHandlers[0][5][0]);
  }
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
...
int GetEntrySignal(void)
  {
   // Stochastic Signal (16, 12, 3)
   double ind0buffer0[]; CopyBuffer(indHandlers[0][0][0], MAIN_LINE,   1, 2, ind0buffer0);
   double ind0buffer1[]; CopyBuffer(indHandlers[0][0][0], SIGNAL_LINE, 1, 2, ind0buffer1);
...
void ManageClose(void)
  {
   // Envelopes (Close, Simple, 52, 0.35)
   double ind4buffer0[]; CopyBuffer(indHandlers[0][4][0], 0, 1, 2, ind4buffer0);
   double ind4buffer1[]; CopyBuffer(indHandlers[0][4][0], 1, 1, 2, ind4buffer1);
   ...
   bool   ind5long  = ind5val1 > Ind5Param0 + sigma && ind5val2 < Ind5Param0 - sigma;
   bool   ind5short = ind5val1 < -Ind5Param0 - sigma && ind5val2 > -Ind5Param0 + sigma;

   if( (posType == OP_BUY  && (ind4long || ind5long)) ||
        (posType == OP_SELL && (ind4short || ind5short)) )
      ClosePosition();
  }
//+------------------------------------------------------------------+
//|                                                                  |
...
//+------------------------------------------------------------------+
/*STRATEGY MARKET Premium Data; AUDJPY; M5 */
/*STRATEGY CODE {"properties":{"entryLots":1,"tradeDirectionMode":0,"oppositeEntrySignal":0,"stopLoss":34,"takeProfit":49,"useStopLoss":true,"useTakeProfit":true,"isTrailingStop":false},"openFilters":[{"name":"Stochastic Signal","listIndexes":[2,0,0,0,0],"numValues":[16,12,3,0,0,0]},{"name":"ADX","listIndexes":[1,0,0,0,0],"numValues":[27,0,0,0,0,0]},{"name":"Bulls Power","listIndexes":[5,0,0,0,0],"numValues":[30,-0.0008,0,0,0,0]},{"name":"Bears Power","listIndexes":[0,0,0,0,0],"numValues":[6,0,0,0,0,0]}],"closeFilters":[{"name":"Envelopes","listIndexes":[0,3,0,0,0],"numValues":[52,0.35,0,0,0,0]},{"name":"Accelerator Oscillator","listIndexes":[4,0,0,0,0],"numValues":[0.6779,0,0,0,0,0]}]} */
