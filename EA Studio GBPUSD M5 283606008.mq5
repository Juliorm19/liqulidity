...
static input int    Magic_Number = 283606008; // Magic number
static input double Entry_Amount =     1.00; // Entry lots
       input int    Stop_Loss    =       35; // Stop Loss   (pips)
       input int    Take_Profit  =       46; // Take Profit (pips)

static input string ___0______   = "------"; // --- Williams' Percent Range ---
       input int    Ind0Param0   =       36; // Period
       input double Ind0Param1   =    -51.6; // Level

static input string ___1______   = "------"; // --- Commodity Channel Index ---
       input int    Ind1Param0   =       18; // Period
       input int    Ind1Param1   =       92; // Level

static input string ___2______   = "------"; // --- Moving Averages Crossover ---
       input int    Ind2Param0   =       11; // Fast MA period
       input int    Ind2Param1   =       21; // Slow MA period

static input string ___3______   = "------"; // --- Pin Bar ---
       input int    Ind3Param0   =        4; // Max body part %
       input int    Ind3Param1   =       28; // Min wick part %
...
static input string News_Currencies   = "GBP,USD,EUR"; // News currencies
...
void InitIndicators(void)
  {
   // Williams' Percent Range (36), Level: -51.6
   indHandlers[0][0][0] = iWPR(NULL, 0, Ind0Param0);
   if(Show_inds) ChartIndicatorAdd(0, 1, indHandlers[0][0][0]);
   // Commodity Channel Index (Typical, 18), Level: 92
   indHandlers[0][1][0] = iCCI(NULL, 0, Ind1Param0, PRICE_TYPICAL);
   if(Show_inds) ChartIndicatorAdd(0, 2, indHandlers[0][1][0]);
   // Moving Averages Crossover (Simple, Simple, 11, 21)
   indHandlers[0][2][0] = iMA(NULL, 0, Ind2Param0, 0, MODE_SMA, PRICE_CLOSE);
   if(Show_inds) ChartIndicatorAdd(0, 0, indHandlers[0][2][0]);
   // Moving Averages Crossover (Simple, Simple, 11, 21)
   indHandlers[0][2][1] = iMA(NULL, 0, Ind2Param1, 0, MODE_SMA, PRICE_CLOSE);
   if(Show_inds) ChartIndicatorAdd(0, 0, indHandlers[0][2][1]);
   // Pin Bar (4, 28)
   indHandlers[0][3][0] = -1;
...
int GetEntrySignal(void)
  {
   ...
   double ind2buffer1[]; CopyBuffer(indHandlers[0][2][1], 0, 1, 2, ind2buffer1);
...
void ManageClose(void)
  {
...

   {
      ...
      bool isPinBar = closeOpen < Ind3Param0 * 0.01 * (High(1) - Low(1));
      bool upWick   = High(1) - MathMax(Close(1), Open(1)) > Ind3Param1 * 0.01 * closeOpen;
      bool downWick = MathMin(Close(1), Open(1)) - Low(1)  > Ind3Param1 * 0.01 * closeOpen;
...
//+------------------------------------------------------------------+
/*STRATEGY MARKET Premium Data; GBPUSD; M5 */
/*STRATEGY CODE {"properties":{"entryLots":1,"tradeDirectionMode":0,"oppositeEntrySignal":0,"stopLoss":35,"takeProfit":46,"useStopLoss":true,"useTakeProfit":true,"isTrailingStop":false},"openFilters":[{"name":"Williams' Percent Range","listIndexes":[5,0,0,0,0],"numValues":[36,-51.6,0,0,0,0]},{"name":"Commodity Channel Index","listIndexes":[5,5,0,0,0],"numValues":[18,92,0,0,0,0]},{"name":"Moving Averages Crossover","listIndexes":[3,0,0,0,0],"numValues":[11,21,0,0,0,0]}],"closeFilters":[{"name":"Pin Bar","listIndexes":[0,0,0,0,0],"numValues":[4,28,0,0,0,0]}]} */
