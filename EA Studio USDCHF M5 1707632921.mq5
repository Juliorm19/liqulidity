...
static input int    Magic_Number = 1707632921; // Magic number
static input double Entry_Amount =     1.00; // Entry lots
       input int    Stop_Loss    =       55; // Stop Loss   (pips)
       input int    Take_Profit  =       75; // Take Profit (pips)

static input string ___0______   = "------"; // --- MACD Signal ---
       input int    Ind0Param0   =        5; // Fast EMA
       input int    Ind0Param1   =       19; // Slow EMA
...
static input string ___1______   = "------"; // --- Moving Averages Crossover ---
       ...
       input int    Ind1Param1   =       29; // Slow MA period

static input string ___2______   = "------"; // --- Pin Bar ---
       input int    Ind2Param0   =        3; // Max body part %
       input int    Ind2Param1   =       38; // Min wick part %
...
void InitIndicators(void)
  {
   // MACD Signal (Close, 5, 19, 5)
   indHandlers[0][0][0] = iMACD(NULL, 0,  Ind0Param0,  Ind0Param1,  Ind0Param2, PRICE_CLOSE);
   if(Show_inds) ChartIndicatorAdd(0, 1, indHandlers[0][0][0]);
   // Moving Averages Crossover (Simple, Simple, 14, 29)
   ...
   if(Show_inds) ChartIndicatorAdd(0, 0, indHandlers[0][1][1]);
   // Pin Bar (3, 38)
   indHandlers[0][2][0] = -1;
...
int GetEntrySignal(void)
  {
   // MACD Signal (Close, 5, 19, 5)
   double ind0buffer0[]; CopyBuffer(indHandlers[0][0][0], 0, 1, 3, ind0buffer0);
   double ind0buffer1[]; CopyBuffer(indHandlers[0][0][0], 1, 1, 3, ind0buffer1);
...
/*STRATEGY CODE {"properties":{"entryLots":1,"tradeDirectionMode":0,"oppositeEntrySignal":0,"stopLoss":55,"takeProfit":75,"useStopLoss":true,"useTakeProfit":true,"isTrailingStop":false},"openFilters":[{"name":"MACD Signal","listIndexes":[3,3,0,0,0],"numValues":[5,19,5,0,0,0]},{"name":"Moving Averages Crossover","listIndexes":[0,0,0,0,0],"numValues":[14,29,0,0,0,0]}],"closeFilters":[{"name":"Pin Bar","listIndexes":[0,0,0,0,0],"numValues":[3,38,0,0,0,0]}]} */
